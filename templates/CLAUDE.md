...
## Test file
tests/test_NNN_name.py (already written — implementation must pass all tests)
```
...
Tests are the definition of "done." The orchestrator runs the task-specific test
file (`pytest tests/test_NNN_name.py -x`) after every Aider attempt. A task is
considered passing only when all its tests pass and at least one test was collected.

### Rules for test files

- **Name must match the spec (with underscores):** `specs/task-001-user-auth.md` →
  `tests/test_001_user_auth.py`. Convert hyphens to underscores — Python cannot
  import files with hyphens, so pytest will not discover them.
...
6. Run `pytest tests/test_NNN_name.py -v` to confirm all tests pass.
...
4. Make one of three decisions and act on it immediately:
   ...
   **FIX THEN MERGE** — tests pass but a constraint was missed or there is a
   ...
   `pytest tests/test_NNN_name.py -v` to confirm, commit the fix, then
...
│   ├── test_001_name.py   ← test files you generate (underscores, not hyphens!)
│   └── test_002_name.py
└── src/
    ├── __init__.py
...
| "Add a task for Y" | Write a new `specs/task-NNN-y.md` + `tests/test_NNN_y.py`, update SpecsReadMe.md |
